    ...
    <section className="relative overflow-hidden bg-slate-950 py-20 sm:py-28 text-white border-y border-slate-800/80">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative">
          ...
          {/* Edge Gradient Masks */}
          <div className="pointer-events-none absolute inset-y-0 left-0 z-20 w-16 sm:w-28 bg-gradient-to-r from-slate-950 via-slate-950/80 to-transparent" />
          <div className="pointer-events-none absolute inset-y-0 right-0 z-20 w-16 sm:w-28 bg-gradient-to-l from-slate-950 via-slate-950/80 to-transparent" />

          {/* Marquee Track Container (Hover pauses animation) */}
          <div className="group flex overflow-hidden select-none py-3">
            {/* Track 1 */}
            <div className="flex shrink-0 animate-marquee items-center gap-5 sm:gap-6 group-hover:[animation-play-state:paused]">
              {marqueeItems.map((item, idx) => {
                const isPurple = item.accent === 'purple';
                return (
                  <div
                    key={`track-1-${idx}`}
                    className="flex items-center gap-4 rounded-2xl border border-slate-800/90 bg-slate-900/75 px-5 py-3.5 backdrop-blur-md shadow-lg transition-all duration-300 hover:-translate-y-1 hover:border-cyan-500/50 hover:bg-slate-800/90 hover:shadow-cyan-500/10 hover:shadow-xl"
                  >
                    {/* SVG Icon Box */}
                    <div
                    ...
                    {/* Authority Info */}
                    <div>
                      <div className="font-extrabold text-sm text-white tracking-wide whitespace-nowrap">
                        {item.name}
                      </div>
                      <div className="text-xs text-slate-400 whitespace-nowrap mt-0.5 font-medium">
                        {item.sub}
                      </div>
                    </div>

                    {/* Verified Tick Badge */}
                    <div className="ml-2 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-400">
            ...
            <div
              aria-hidden="true"
              className="flex shrink-0 animate-marquee items-center gap-5 sm:gap-6 group-hover:[animation-play-state:paused]"
            >
              {marqueeItems.map((item, idx) => {
                const isPurple = item.accent === 'purple';
                return (
      ...
      {/* Embedded Continuous CSS Marquee Animation */}
      <style jsx global>{`
        @keyframes marquee {
          0% {
